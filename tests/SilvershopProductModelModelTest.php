...
final class SilvershopProductModelModelTest extends SapphireTest
{
    protected static $fixture_file = 'vendor/silvershop/core/tests/php/Fixtures/shop.yml';

    public function testNew(): void
    {
        $productCategory = $this->objFromFixture(ProductCategory::class, "electronics");
        $productModel = ProductModel::create(['Title' => 'Freighter', 'Description' => 'Transportation across the Galaxy', 'ProductCategoryID' => $productCategory->ID]);
        $id = $productModel->write();
        ...
        $this->assertSame(
            (int) $productCategory->ID,
            $product_model->ProductCategoryID,
            'The ProductCategoryID is ' . $productCategory->ID
        );
    }
